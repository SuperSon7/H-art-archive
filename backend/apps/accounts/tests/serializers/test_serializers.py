...
class TestLoginSerializer:
    def test_valid_login_data(self, active_user):
        """유효한 로그인 데이터 테스트"""
        data = {"email": "active@example.com", "password": "testuser123"}
...
class TestProfileImageUploadSerializer:
    def test_valid_image_data(self):
        """유효한 이미지 업로드 데이터 테스트"""
        data = {"filename": "profile.jpg", "content_type": "image/jpeg"}
        serializer = S3ImageUploadSerializer(data=data)
        assert serializer.is_valid()

    def test_invalid_content_type(self):
        """잘못된 content type 테스트"""
        data = {"filename": "profile.pdf", "content_type": "application/pdf"}
        serializer = S3ImageUploadSerializer(data=data)
        assert not serializer.is_valid()
        assert "non_field_errors" in serializer.errors
    ...
    def test_mismatched_extension_and_content_type(self):
        ...
        data = {"filename": "profile.jpg", "content_type": "image/png"}
        serializer = S3ImageUploadSerializer(data=data)
        assert not serializer.is_valid()
...
class TestSocialLoginSerializer:
    ...
    def test_invalid_provider(self):
        """잘못된 provider 테스트"""
        data = {"code": "auth_code_123", "provider": "facebook"}
        serializer = SocialLoginSerializer(data=data)
        assert not serializer.is_valid()
