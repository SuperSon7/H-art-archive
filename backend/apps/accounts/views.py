...
class SendVerificationEmailView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response({"success": True, "message": "인증 이메일을 발송하였습니다."})
            ...
            if settings.DEBUG:
                ...
                return Response(
                    {"success": False, "message": "이메일 발송에 실패했습니다."}, status=500
                )
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            if user.refresh_token != refresh_token:
                return Response({"error": "already fired or changed token"}, status=401)

            return Response(
                {"success": True, "access_token": new_access_token}, status=status.HTTP_200_OK
            )
...
class UserInfoViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=["post"], url_path="profile-image", url_name="profile-image")
    def generate_profile_image_url(self, request, pk=None) -> Response:
        ...
        try:
            s3_key = s3_key_for_upload(category="profiles", object_id=user.id, ext=ext)
        except Exception as e:
            logger.error(f"Failed to generate s3_key for {user.id}: {str(e)}")
            raise ValidationError("Failed to generate s3_key")

        try:
            bucket = getattr(settings, "AWS_S3_PROFILE_BUCKET", None)
            if not bucket:
                return JsonResponse({"error": "AWS_S3_PROFILE_BUCKET not configured"})

            url, s3_key = create_presigned_url(
                bucket=bucket, key=s3_key, content_type=serializer.validated_data["content_type"]
            )
        except Exception as e:
        ...
        return Response({"upload_url": url, "s3_key": s3_key}, status=status.HTTP_200_OK)

    @action(
        detail=True, methods=["post"], url_path="profile-image/save", url_name="profile-image-save"
    )
    def save_profile_image_url(self, request, pk=None) -> Response:
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            tokens = login_with_social(
                provider=data["provider"], code=data["code"], redirect_uri=data.get("redirect_uri")
            )
